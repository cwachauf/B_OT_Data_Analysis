...
{
  real lts_open_nf[N_open];
  real lts_open[N_open];
  real lts_closed_nf[N_closed];
  real lts_closed[N_closed];
  for(i in 1:N_open)
  {
    lts_open_nf[i] <- LifetimeFromForce_BS_Open(tau0_open,P,L_trans,L_open,Temp,k_eff,forces_open[i]);
    lts_open[i] <- gamma_rng(N_eff,1.0/lts_open_nf[i])/N_eff;
  }
  for(i in 1:N_closed)
  {
    lts_closed_nf[i] <- LifetimeFromForce_BS_Closed(tau0_closed,P,L_trans,L_open,Temp,k_eff,forces_closed[i]);
    lts_closed[i] <- gamma_rng(N_eff,1.0/lts_closed_nf[i])/N_eff;
  }
}